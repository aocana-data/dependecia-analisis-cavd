...
from .modules.athena_module.rounding_format import round_down
...
class DataCleaner:
    ...
    # SKIP ANALISIS DE COMPLETITUD Y EXACTITUD GENERAL
    # TODO>> MEJORAR EL MOTOR DE ANALISIS
    ...
    def __init__(self, **kwargs) -> None:
        ...
        if self.SKIP_GENERAL == 1:
            add_ = f"""
            >>>    SE OMITE EL ANALISIS DE TODO EL DATAFRAME CON EL OBJETO DE OBTENER SOLAMENTE LOS DATOS
               POR DEFECTO, O SE ESTA REALIZANDO UN ANALISIS POR AGRUPACION
    ...
    def get_agg_completitud(self, **kwargs):
        """
        La mayoria de los parametros se pueden setear desde el config
        o usando estos parametros.

        @params
            * grouped_by:list 
                -   por defecto se tiene que generar por le usuario 
            * lte:int 
                -   filtro de completitud general menor a
            * gte:int 
                -   filtro de completitud general mayor a
        ...
        completitud = [
            "registros_totales",   "registros_completos",   "completitud",   "registros_completos_por_filas",   "completitud_por_filas"
        ]

        completitud_df = self.resumen_agg_df\
            .loc[:, self.resumen_agg_df.columns.get_level_values(1).isin(completitud)]

        completitud_nombre, _ = NOMBRES_GENERALES

        lte = kwargs.get("lte", None)
        gte = kwargs.get("gte", None)

        comp_filter = completitud_df[completitud_nombre]["completitud_por_filas"]

        if bool(lte):
            return completitud_df[comp_filter <= lte]
        elif bool(gte):
            return completitud_df[comp_filter >= gte]

        else:
            print(">> COMPLETITUD SIN FILTROS")
    ...
    def get_agg_exactitud(self, **kwargs):
        ...
        @params
            * grouped_by:list 
                -   por defecto se tiene que generar por le usuario 
            * lte:int 
                -   filtro de exactitud general menor a
            * gte:int 
                -   filtro de exactitud general mayor a
        ...
        exactitud = [
            "registros_totales",   "registros_exactos",   "exactitud",   "registros_exactos_por_filas",   "exactitud_por_filas"]

        exactitud_df = self.resumen_agg_df\
            .loc[:, self.resumen_agg_df.columns.get_level_values(1).isin(exactitud)]

        _, exactitud_nombre = NOMBRES_GENERALES

        lte = kwargs.get("lte", None)
        gte = kwargs.get("gte", None)

        exac_filter = exactitud_df[exactitud_nombre]["exactitud_por_filas"]

        if bool(lte):
            return exactitud_df[exac_filter <= lte]
        elif bool(gte):
            return exactitud_df[exac_filter >= gte]

        else:
            print(">> EXACTITUD SIN FILTROS")
            return exactitud_df
    ...
    def get_agg_resumen(self, **kwargs):
        ...
        @params
            *   grouped_by: list 
                -   por defecto se tiene que generar por le usuario 
            *   not_nullish: dict 
                -   permite realizar un mapping de valores nulos 
                ejemplo: {"base_origen" : "valor no nulo"}
            *   add_general:bool
                -   permite que se agregue el valor de completitud general

            *   exactitud_lte : int 
                -   filtrado de valores de exactitud general menores a
            *   exactitud_gte : int 
                -   filtrado de valores de exactitud general mayores a
            *   completitud_lte :   int 
                -   filtrado de valores de completitud general menores a
            *   completitud_gte :   int 
                -   filtrado de valores de completitud general mayores a
        ...
        completitud_nombre, exactitud_nombre = NOMBRES_GENERALES

        exactitud_lte = kwargs.get("exactitud_lte", None)
        exactitud_gte = kwargs.get("exactitud_gte", None)

        completitud_lte = kwargs.get("completitud_lte", None)
        completitud_gte = kwargs.get("completitud_gte", None)
        ...
        if (self.resumen_agg_df is not None and (self.config.get("grouped_by", kwargs.get("grouped_by", None)) == self.grouped_by)):
            ...
            comp_filter = df[exactitud_nombre]["exactitud_por_filas"]
            exact_filter = df[completitud_nombre]["completitud_por_filas"]

            if bool(exactitud_lte):
                return df[comp_filter <= exactitud_lte]

            if bool(exactitud_gte):
                return df[comp_filter >= exactitud_gte]

            if bool(completitud_lte):
                return df[exact_filter <= completitud_lte]

            if bool(completitud_gte):
                return df[exact_filter >= completitud_gte]
        ...
        options = {
            ...
            "chars_null": self.chars_omitir_exactitud,
            "rules": self.rules
        }
        ...
        if self.SKIP_GENERAL == 1:
            print(">> SE GUARDAN LAS CONFIGURACIONES REALIZADAS")
    ...
    def __get_agg_criterio_minimo(self, **kwargs):
        ...
        df = self.resumen_agg_df.iloc[:, -4:]
        cols = ["completitud_por_filas", "exactitud_por_filas"]

        df['criterio_minimo'] = df.loc[:, df.columns.get_level_values(1).isin(
            cols)].apply(lambda x: round_down(np.prod(x)/100, 2), axis=1)

        return df

    def criterio_minimos_agg(self, **kwargs):
        with HiddenPrints():
        ...
        if k == "criterio_lte":
            return df[df["criterio_minimo"] <= v]
        if k == "criterio_gte":
            return df[df["criterio_minimo"] >= v]
    ...
    def get_exactitud(self) -> any:
        ...
        try:
            ...
            if self.SKIP_GENERAL == 1:
                print(
                    ">>> SE OMITE EL ANALISIS DE EXACTITUD PARA OBTENER LOS VALORES POR DEFECTO DE COMPLETITUD")
                return
    ...
    def get_criterios_minimos_resumen(self, **kwargs):
        ...
        try:
            ...
            print("Los criterios minimos fueron ejecutados exitosamente")
    ...
    def __to_csv_agg_data(self, **kwargs):
        ...
        FILE_GROUPED = [
            path / f"GROUPED_RESUMEN_GENERAL_{FILE_NAME}.csv",   path / f"GROUPED_RESUMEN_COMPLETITUD_{FILE_NAME}.csv",   path /
            f"GROUPED_RESUMEN_EXACTITUD_{FILE_NAME}.csv",   path /
            f"GROUPED_RESUMEN_CRITERIO_MINIMO_{FILE_NAME}.csv"
        ]

        DF_DATA = [
            self.get_agg_resumen().reset_index(),   self.get_agg_completitud().reset_index(
            ),   self.get_agg_exactitud().reset_index(),   self.criterio_minimos_agg().reset_index()
        ]
