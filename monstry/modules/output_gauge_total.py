...
def score(dataframe,column):
    """
    :params
        pandas_dataframe resumen del analisis
    
    :return 
        retorno de los valores de la media
    """
        
    col =   dataframe[[column]]
    completitud = list(col.to_numpy())
    
    return round(
            float(sum(completitud)/len(completitud))
            ,   2
    )


def get_limits(limits,default):
    if not os.getenv(limits,None):
        return default
    
    from_env:list = os.getenv(limits).split(",")
    
    return [int(limit) for limit in from_env]
...
def gauge(**kwargs): 
    ...
    for ang, c in zip(ang_range, colors): 
        ...
        # arcs
        patches.append(Wedge((0.,0.), .4, *ang, width=0.08, facecolor=c, lw=2, alpha=0.5))
    ...
    labels = [f"{label}\n{values[0]}% - {values[1]}%" for label,values in zip(labels,[r[1] for r in PARAMS.items()][::-1])]
